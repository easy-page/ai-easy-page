import { ComponentPropsWithoutRef, ComponentType, FC, memo, useContext } from 'react';
import { PreContext, PreOverride, useIsMarkdownCodeBlock } from './PreOverride';
...
const CodeBlockOverride: FC<CodeOverrideProps> = ({
    ...
    const WrappedPre: PreComponent = useCallbackRef((props) => <Pre {...getPreProps(props)} />);

    const getCodeProps = withDefaultProps<any>(codeProps);
    const WrappedCode: CodeComponent = useCallbackRef((props) => <Code {...getCodeProps(props)} />);

    const language = /language-(\w+)/.exec(codeProps.className || '')?.[1] ?? '';

    // if the code content is not string (due to rehype plugins), return a default code block
    if (typeof children !== 'string') {
        console.log('SyntaxHighlighter 1 11', children);
        return (
            <DefaultCodeBlockContent
                components={{ Pre: PreOverride, Code: WrappedCode }}
                code={children}
            />
        );
    }
    ...
    console.log('SyntaxHighlighter 1', componentsByLanguage[language], FallbackSyntaxHighlighter);
    return (
        <DefaultCodeBlock
            components={{
                Pre: PreOverride,
...
const CodeOverrideImpl: FC<CodeOverrideProps> = ({
    ...
    console.log('SyntaxHighlighter 2', isCodeBlock);
