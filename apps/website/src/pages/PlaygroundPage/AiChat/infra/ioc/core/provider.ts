...
export abstract class FrameworkProvider {
    ...
    createEntity = <T extends Entity<any>, Props extends T extends Component<infer P> ? P : never>(
        identifier: GeneralIdentifier<T>,
    ...
    createScope = <T extends Scope<any>, Props extends T extends Component<infer P> ? P : never>(
...
class Resolver extends FrameworkProvider {
    ...
    getRaw(
        identifier: IdentifierValue,
        { sameScope = false, optional = false, noCache = false, props }: ResolveOptions = {},
    ) {
    ...
    track(identifier: IdentifierValue): Resolver {
        ...
        const circular = this.stack.find(
            (i) =>
                i.identifierName === identifier.identifierName && i.variant === identifier.variant,
        );
        if (circular) {
