...
import { InputToolsConfig, SenceInput, SenceInputComponentOperation } from '@/common/interfaces';
...
const InputToolItem: Record<InputToolsEnum, (tool: InputToolConfig) => React.ReactNode> = {
    ...
    [InputToolsEnum.MoreSkills]: (tool) => <MoreSkillsBtn disabled={tool.disabled} />,
};
...
const OperationItem: Partial<
    Record<
        SenceOperationEnum,
        React.FC<{ onChange: (val: string) => void; operation: SenceInputComponentOperation }>
    >
> = {
    [SenceOperationEnum.DialogInput]: ({ onChange, operation }) => (
        <DialogInput onChange={onChange} {...(operation.dialogInputConfig || ({} as any))} />
    ),
...
export const ToolComponent = ({ tool }: { tool: InputToolsEnum | InputToolConfig }) => {
    ...
    return (
        <div className="flex flex-row items-center justify-between">
            ...
            <div className="flex flex-row gap-2 items-center">
                ...
                <SendBtn onSend={onSend} chatMode={chatMode} editor={editor} disabledSend={false} />
            </div>
        </div>
    );
};
